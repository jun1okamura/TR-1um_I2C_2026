...
TECH_PY_DIR = _cfg.pdk_tech_python()
...
IN_GDS = _cfg.artifact("i2c_slave_async_nrow_fm_placement.gds")
OUT_GDS = _cfg.artifact("i2c_slave_async_nrow_fm_routed.gds")
TOP_CELL_NAME = _cfg.TOP_CELL_NAME
...
# ここで効くのは (a) 各行の右端の照合 (b) in_bounds による行またぎ x の上限。
# (b) は **行スタックの幅でなければならない**。行またぎはマクロの上では
# 起こり得ないし、マクロの右の空き地で起こしても意味が無い。
ROW_WIDTH_UM = _cfg.ROW_WIDTH_UM

# --- TD4 移植 (3a): ピンが片側 1 辺にしか出ていないマクロ ---------------------
# ここに挙げたインスタンスにピンを持つネットは、下（ch[0]）からしか出られない。
# 理由と処理は main() の「TD4 移植 (3)」を参照。
DOWN_FACING_INSTS = set(getattr(_cfg, "DOWN_FACING_INSTS", ()))
...
def main(placement_json=PLACEMENT_JSON, in_gds=IN_GDS, out_gds=OUT_GDS,
    ...
    if pin_map_path is None:
        pin_map_path = _cfg.artifact("pin_map_nrow_fm.json")
    if net_shapes_path is None:
        net_shapes_path = _cfg.artifact("net_shapes_nrow_fm.json")
    ...
    priority_corridor_x = []
    for inst in rows[0]:
        # TD4 移植 (4): コリドーのセル種を FILL2 に限定しない。
        # FILL2 (10.8 um = 2 トラック) だと上側のトラックの via パッド
        # (半幅 1.7 + 隙間 2.0) が隣のセルへ 1.0 um はみ出し、隣に M2 が
        # あると使えない。FILL3 (16.2 um = 3 トラック) にすると**真ん中の
        # 1 本が必ず両側とも空く**。
    ...
    # --- TD4 移植 (3): 下向きピンのマクロは必ず ch[0] から出す -----------------
    # REG8x16 の信号ピン 21 本は**マクロの下辺 1 列**にあり、その y は row0 の
    # 下端 +1.1 um。つまり出口は ch[0] しかない。上へ出そうとすると自分の
    # 上に 933 um のマクロ本体が乗っていて M2 が通らない。
    #
    # ところが既定の分類はピンの「行」しか見ないので、マクロと row1 のセルを
    # 繋ぐネットは adjacent_pair として **ch[1]（row0 の上）** に割り振られ、
    # マクロのピンから上へ出ようとして詰まる（draw_jog が
    # 「x=1223.1 で clear な departure leg が見つからない」と言い続ける症状）。
    #
    # そこでマクロにピンを持つネットは**問答無用で spanning 扱い・legal=[0]**
    ...
    per_row_spine_events_path = _cfg.artifact("per_row_spine_events_nrow_fm.json")
    ...
    if force_jog_events_path is None:
        force_jog_events_path = _cfg.artifact("force_jog_events_nrow_fm.json")
