...
OUT_GDS = _cfg.artifact("i2c_slave_async_nrow_fm_placement.gds")
...
def main(placement_json=PLACEMENT_JSON, out_gds=OUT_GDS, ch_heights=None):
    ...
    # TD4 移植: コア枠の注釈は**コア幅**で描く（行スタック幅ではない）。
    # 右の 399.6 um はマクロで、行には属さないが確かにコアの一部。
    core_w = placement.get("core_w", row_w)
    top.shapes(ann).insert(box(0, 0, core_w, core_h))
