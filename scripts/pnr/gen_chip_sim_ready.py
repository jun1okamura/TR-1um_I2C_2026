...
## 直すところ（V10 の `gen_chip_sim_ready_v9/v10.py` と同じ 5 点）
 ...
 5. **素子を 1 個も持たないセル**（OpenSUSI ロゴ、ダイシール、via の
    ...
    **ポートが 2 本以上あるものは落とさない**（中でネットを繋いでいる
    可能性があるので）。
...
def drop_empty_subckts(text):
    ...
    2 本以上あるものは中でネットを繋いでいるかもしれないので触らない。
