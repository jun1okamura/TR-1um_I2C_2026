...
IN_GDS = _cfg.artifact("i2c_slave_async_nrow_fm_v7rr_routed.gds")
OUT_GDS = _cfg.artifact("v7v2_step_6_top_pins_routed.gds")
...
def main(placement_json=PLACEMENT_JSON, in_gds=IN_GDS, out_gds=OUT_GDS, ch_heights=CH_HEIGHTS,
    ...
    row3_bound = row_y0[len(row_y0) - 1] + row_h       # row3's own top edge = channel4's bottom edge
    ...
    if len(row_y0) >= 3:
        ...
        ch2_lo, ch2_hi = row_y0[-1] + row_h, core_h  # channel2 band (between row1 and row2)
