...
def _scan_ports(net_path):
    ...
    # --- TD4 移植 (5): エスケープ識別子の別名を拾う ---------------------------
    # 原本の正規表現は右辺を `[A-Za-z_][\w$]*` に限っていたので
    #   assign out_port = \u_core.reg_out ;
    #   assign cflag_o  = \u_core.cflag ;
    #   assign pc       = \u_core.pc ;
    # を**1 本も拾えなかった**。結果 gather_pins が out_port[0..3] と cflag_o の
    # ネットを見つけられず、step8 が「MISSING 5」で止まる。
    # Verilog のエスケープ識別子は `\` から空白までが 1 トークンで、ビット指定は
    # `\u_core.pc [0]` と**空白を挟んで**書かれる（Yosys の出力そのまま。
    # placement JSON のネット名もこの綴り）。
...
def bus_bit_net_name(bus, i, resolver=None):
    ...
    # TD4 移植 (5): エスケープ識別子のビット指定は `\name [i]`（空白を挟む）。
    net = f"{base} [{i}]" if base.startswith("\\") else f"{base}[{i}]"
    return resolver(net) if resolver else net
...
if __name__ == "__main__":
    PLACEMENT_JSON = _cfg.artifact("placement_nrow_fm_v6.json")
    IN_GDS = _cfg.artifact("v6_step_7_minheight_compressed.gds")
    OUT_GDS = _cfg.artifact("v6_step_7_top_pins_highlighted.gds")
