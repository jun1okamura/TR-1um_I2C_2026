...
def main(in_gds, compaction_info_path, out_gds, pin_map_in=None, pin_map_out=None,
         ...
         force_jog_events_out=None, extra_protect=()):
    ...
    # トラックが保護区間のすぐ脇まで寄ってしまう。実測: 圧縮後の ch2 で
    # トラックが 540.6 / 543.3 / 544.8 / 547.8 / 550.8 / 553.8 と 1.5〜3.0 µm
    # 間隔に潰れ、M1 の間隔違反が 11 件出た（圧縮前は 0 件）。
    # 原因は `NO_BOTTOM_PORTS` で row0 のポートを左右へ回したことで、
    ...
    # そこへ降りてくる配線は潰れた分だけ短くなり、**ピンから外れる**。
    # 今の設計ではたまたま潰す場所がマクロの外だけだったが、チャネル予算を
    # 変えれば簡単に中へ入る。identity を強制して事故を止める。
    if extra_protect:
        merged, cur = [], None
        for a, b in sorted(list(protect_y) + [tuple(x) for x in extra_protect]):
            if cur and a <= cur[1]:
...
if __name__ == "__main__":
    IN_GDS = _cfg.artifact("i2c_slave_async_nrow_fm_v6_minheight_routed.gds")
    COMPACTION_INFO = "/tmp/v6_compaction_info.json"
    OUT_GDS = _cfg.artifact("i2c_slave_async_nrow_fm_v6_squeezed_routed.gds")
    PIN_MAP_IN = _cfg.artifact("i2c_slave_async_nrow_fm_v6_minheight_pin_map.json")
    PIN_MAP_OUT = _cfg.artifact("i2c_slave_async_nrow_fm_v6_squeezed_pin_map.json")
    NET_SHAPES_IN = _cfg.artifact("i2c_slave_async_nrow_fm_v6_minheight_net_shapes.json")
    NET_SHAPES_OUT = _cfg.artifact("i2c_slave_async_nrow_fm_v6_squeezed_net_shapes.json")
    FJ_IN = _cfg.artifact("i2c_slave_async_nrow_fm_v6_minheight_force_jog_events.json")
    FJ_OUT = _cfg.artifact("i2c_slave_async_nrow_fm_v6_squeezed_force_jog_events.json")
