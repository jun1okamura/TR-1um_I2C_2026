...
出どころ（`lef/simulation/README` の分類でいう **(B) 検証済み抽出を golden 凍結**）:
  ...
  **レイアウトから抽出したものを凍結した**ものであり、これに対する LVS は
  「レイアウト vs そのレイアウトから作った網」なので**それ自体は何も証明しない**。
  意味があるのは次の 2 つ:
    - フレームを取り込んだチップ全体の LVS で、フレーム部分が既知の網と
      一致することを確かめられる（回帰検査）
...
def emit(nl, top, gds):
    ...
    L = [f"* {top} — LVS ソースネットリスト（フレーム）",
         ...
         "*   「レイアウト vs そのレイアウトから作った網」であり、それ自体は何も",
         ...
         "*",
         "* 書き出したあと scripts/lvs_check.py で抽出網と照合して一致を確認済み",
